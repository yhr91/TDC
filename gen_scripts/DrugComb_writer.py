...
# Subset columns
drugcomb_subset = drugcomb.dropna(0).loc[:,['drug_row', 'drug_col', 'cell_line_name', 'css',
                          'synergy_zip', 'synergy_bliss', 'synergy_loewe', 'synergy_hsa']]

# Take the average across replicates
drugcomb_subset_mean = drugcomb_subset.groupby(['drug_row','drug_col','cell_line_name']).mean().reset_index()
...
all_cmpds = list(set(drugcomb_subset.drug_row.values).union(set(drugcomb_subset.drug_col.values)))
smiles_map = {c:get_SMILES(c) for c in all_cmpds}


# Create new columns for SMILES
drugcomb_subset_mean['smiles_row'] = [smiles_map[v] for v in drugcomb_subset_mean.drug_row.values]
drugcomb_subset_mean['smiles_col'] = [smiles_map[v] for v in drugcomb_subset_mean.drug_col.values]

# Rename columns
drugcomb_subset_mean = drugcomb_subset_mean.rename(columns={'drug_row':'Drug1_ID', 'drug_col':'Drug2_ID', 'cell_line_name':'Cell_Line_ID',
...
def get_feat_dict(path, start_col):    
    ...
    feat_df = feat_df.rename(columns = {'HCT-116':'HCT116', 'UACC-62':'UACC62', 'A549/ATCC': 'A549',
                                         'NCI-H23':'NCIH23', 'OVCAR-3':'OVCAR3'})
...
protein_feats = get_feat_dict('./NCI/Protein__SWATH_(Mass_spectrometry)_Protein.xls', start_col=9)
expression_feats = get_feat_dict('./NCI/RNA__RNA_seq_composite_expression.xls', start_col=6)
microRNA_feats = get_feat_dict('./NCI/RNA__microRNA_OSU_V3_chip_log2.xls', start_col=9)

# Add genomic feature columns to the dataframe 
drugcomb_subset_mean['protein_feats'] = [protein_feats[v] for v in drugcomb_subset_mean['Cell_Line_ID'].values]
drugcomb_subset_mean['expression_feats'] = [expression_feats[v] for v in drugcomb_subset_mean['Cell_Line_ID'].values]
drugcomb_subset_mean['microRNA_feats'] = [microRNA_feats[v] for v in drugcomb_subset_mean['Cell_Line_ID'].values]
...
drugcomb = drugcomb.rename(columns = {'css':'CSS', 'synergy_zip':'Synergy_ZIP', 'synergy_bliss': 'Synergy_Bliss',
                           'synergy_loewe': 'Synergy_Loewe', 'synergy_hsa': 'Synergy_HSA',
...
drugcomb = drugcomb.drop(['CellLine_Feat_Expression','CellLine_Feat_microRNA','CellLine_Feat_Proteome'], axis=1)
